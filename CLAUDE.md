...
**Getting a DB schema into planoo — two paths, one preferred:** Onboarding originally required running `scripts/agent.ts` against a real database (step 3). Live usage showed this was the biggest friction point, so the default is now the in-browser **schema builder** (`DesignedTable`/`DesignedColumn` in `prisma/schema.prisma`, UI in `src/components/canvas/SchemaBuilder.tsx`, CRUD API under `src/app/api/schema/`) — no DB connection, no CLI, and it exports directly to `.sql` (`src/lib/schema-export.ts`, `GET /api/schema/export`). `src/lib/schema-source.ts`'s `getDbColumns()` is the single place that decides which source feeds the matcher: designed tables if any exist, else falls back to the last agent-pushed snapshot. The agent path still works end-to-end (nothing was deleted), it's just not surfaced in `Dashboard.tsx` anymore — see `TODOS.md` for re-exposing it as an advanced option.
